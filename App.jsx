...
const META_CMV = 35; // % ideal para hamburguerias
...
export default function App() {
  ...
  // Custo de um lanche com preços atuais
        ...
        <DarkCard>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 14 }}>
            {porCat.map(([cat, val]) => {
              ...
              const isProteina = cat === "Proteínas";
              const barColor = isProteina && barPct > 50 ? "#ef4444" : "#f97316";
              return (
                <div key={cat}>
...
function TelaCardapio({ lanches, setLanches, insumos, calcCusto }) {
  ...
  function abrirEdicao(l) {
    setModo("form"); setEditId(l.id); setNome(l.nome);
    setPreco(l.precoVenda); setItens(l.itens.map(i => ({ ...i, qtd: String(i.qtd) })));
  }
  ...
  if (!insumos.length) return (
    <DarkCard style={{ textAlign: "center", padding: "40px 20px" }}>
      ...
      <div style={{ color: "#57534e", fontSize: 13, marginTop: 4 }}>Vá na aba Insumos para começar.</div>
    </DarkCard>
  );
    ...
    <div>
      ...
      {lanches.length === 0 ? (
        <DarkCard style={{ textAlign: "center", padding: "40px 20px" }}>
          ...
          <div style={{ color: "#57534e", fontSize: 13, marginTop: 4 }}>Crie seu primeiro lanche acima.</div>
        </DarkCard>
      ) : (
